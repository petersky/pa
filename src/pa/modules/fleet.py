...
@router.get("/goal-assigned-session/restart-handoffs")
def list_assigned_restart_handoffs(request: Request) -> dict[str, Any]:
    record = _assigned_local_dispatch(request)
    manager = request.app.state.ctx.require_service("instance_agent")
    return {"handoffs": [
        item.model_dump(mode="json")
        for item in manager.store.list_restart_handoffs(session_id=str(record.session_id))
    ]}
...
class FleetModule(Module):
    ...
    def register_mcp(self, mcp, ctx) -> None:
        from pa.mcp.tools.fleet import register_mcp
