...
(function () {
  ...
  function edgeStatusSummary(edge) {
    ...
    return ["unavailable", "degraded", "stale", "healthy"].filter(function (status) {
  ...
  function renderFleetDetail(kind, id, edgeId) {
    ...
    if (kind === "edge-item") {
      ...
      var watchLink = parent.kind === "supervisor"
        ? '<p><a href="/pull-requests?watch=' +
          encodeURIComponent(watchId) + '">Open watch audit history</a></p>'
        : "";
      panel.innerHTML = '<button type="button" class="ghost small" data-fleet-edge="' +
        ...
        "</dd></dl>" + watchLink +
        "<details open><summary>Exact activity detail</summary><pre>" +
  ...
  function renderFleetTopology() {
    ...
    edges.forEach(function (edge) {
      ...
      var parallelOffset = (parallelIndex - (siblings.length - 1) / 2) * 32;
      ...
      if (edge.source === edge.target) {
        ...
        var canonicalDirection = String(edge.source) <= String(edge.target) ? 1 : -1;
        var routedOffset = parallelOffset * canonicalDirection;
        var controlX = (from.x + to.x) / 2 - dy / length * routedOffset;
        var controlY = (from.y + to.y) / 2 + dx / length * routedOffset;
